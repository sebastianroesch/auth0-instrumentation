/** @typedef {import('./metrics_factory').Metrics} Metrics  */
...
class MetricsAgent {
   ...
   * @param {*} resourceCollector
   */
  constructor(
    type,
    metrics,
    options,
    resourceCollector = require('./resource-collector')
  ) {
  ...
  startResourceCollection(tags) {
    ...
    setInterval(function() {
      const memUsage = agent.resourceCollector.getMemoryUsage();
      ...
      agent.resourceCollector
        .getProcessStats()
        .then(stats => {
          agent.gauge('resources.memory.usage', stats.memory, tags);
          agent.gauge('resources.cpu.usage', stats.cpu, tags);
        })
        .catch(() => {});
